...
def drop_color_message_key(_, __, event_dict: EventDict) -> EventDict:
    ...
    but we don't need it. This processor drops the key from the event dict
    if it exists.
    """
...
def setup_logging(json_logs: bool = False, log_level: str = "DEBUG"):
    ...
    if json_logs:
        ...
        # looks for the `message` key but the pretty ConsoleRenderer looks
        # for `event`
    ...
    def handle_exception(exc_type, exc_value, exc_traceback):
        ...
        (but leave KeyboardInterrupt untouched to allow users to Ctrl+C
        to stop)
